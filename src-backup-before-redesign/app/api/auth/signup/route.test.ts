...
describe("POST /api/auth/signup", () => {
  ...
  it("creates an account, returns 201, and sets a session cookie", async () => {
    ...
    const response = await handler(postJson({ email: "new@example.com", password: "a-strong-password" }));
  ...
  it("rejects a duplicate account with 409", async () => {
    const handler = handlerFor();
    await handler(postJson({ email: "dupe@example.com", password: "a-strong-password" }));
    const second = await handler(
      postJson({ email: "dupe@example.com", password: "a-different-password" }),
    );
  ...
  it("rejects an invalid password with 400", async () => {
    const handler = handlerFor();
    const response = await handler(postJson({ email: "shortpw@example.com", password: "short" }));
  ...
  it("rejects a malformed email with 400", async () => {
    ...
    const response = await handler(
      postJson({ email: "not-an-email", password: "a-strong-password" }),
    );
    expect(response.status).toBe(400);
  });
  ...
  it("never returns a raw session token in the JSON body", async () => {
    const handler = handlerFor();
    const response = await handler(postJson({ email: "leak-check@example.com", password: "a-strong-password" }));
  ...
  it("rate-limits repeated signups from the same IP", async () => {
    ...
    for (let i = 0; i < 5; i += 1) {
      const response = await handler(
        postJson({ email: `rl-${i}@example.com`, password: "a-strong-password" }, headers),
      );
      expect(response.status).toBe(201);
    }
    const blocked = await handler(
      postJson({ email: "rl-blocked@example.com", password: "a-strong-password" }, headers),
    );
