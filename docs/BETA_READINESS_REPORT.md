...
| 7 | Database schema & RLS (Supabase, production-linked) | VERIFIED | `npx supabase migration list --linked` re-run this session: 34/34 migrations `local == remote`, zero drift; RLS deny-all posture re-confirmed by Sprint 19 (one day prior, no schema change since) | No | None |
