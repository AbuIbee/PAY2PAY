...
| 18 | **COMPLETE — uncommitted, awaiting Product Owner review.** All required-functionality items from `docs/sprints/SPRINT_18_AdminSupport_Appeals.md` (internal admin/support/compliance/fraud-reviewer role model with fine-grained capabilities, platform/agreement-level payment/new-agreement/payout restrictions distinct from Sprint 18A's relationship-scoped restriction and Sprint 16's dispute-scoped restriction, retention/legal holds — retention/dispute/fraud_review/litigation/administrative_override — with `hasActiveHold` as the future deletion-job's own gate, support-case management, and a full appeals workflow with database-enforced independent-reviewer separation) implemented in an isolated worktree (`sprint-18-adminsupport-appeals`), branched from `origin/master`'s tip (`be88af3`, the Sprint 18A merge commit). Local `lint` (0 errors), `typecheck` (0 errors — the sole remaining `LayoutProps` finding is the same pre-existing `.next/types` artifact every prior worktree sprint has independently documented), `test` (708/708, up from 663 at the end of Sprint 18A — 45 net new across 6 new files), and `build` (`next build`, Turbopack) all pass. `drizzle-kit check` confirms the new migration (`0021_ancient_roughhouse.sql`) is internally consistent; RLS + `REVOKE` confirmed present on all 5 new tables (`admin_role_assignment`, `retention_hold`, `admin_restriction`, `support_case`, `appeal`) — the `REVOKE` lines were added by hand, matching every prior migration's pattern; the `appeal_reviewer_not_original_decision_maker` CHECK constraint and both partial unique indexes (`admin_role_assignment_active_user_unique`, `admin_restriction_active_target_unique`) are hand-verified present in the generated SQL. Migration is purely additive: 6 new enums, 5 new tables, zero altered/dropped columns on any existing table — this is the first sprint since 18A with no additive columns on prior-sprint tables at all. Sprints 1–17 and Sprint 18A's own tests all still pass unchanged — no prior sprint's table, column, or service method was modified, only additively extended (one new `NotificationEventType` member, `appeal_decided`). **No live payment-provider/KYC integration and no scheduled deletion/minimization job exists anywhere in this codebase — the retention-hold mechanism is fully correct and ready but has no real deletion-job caller yet, honestly documented as a known limitation, not silently assumed complete.** **Not yet committed, not pushed, no PR opened, no CI run, no Vercel preview** — deferred until Product Owner review of this status entry, per this session's explicit instruction. Sprint 19 was **not** started. See "Sprint 18 implementation notes" below for the full architectural-boundary rationale (Sprint 18 vs. Sprint 18A vs. Sprint 16 restriction mechanisms), design decisions, and known limitations. |
...
**Architectural boundary — three restriction mechanisms now coexist in this codebase, deliberately never merged.** This sprint's own instruction was explicit: "do not duplicate or replace Sprint 18A relationship restriction behavior unless the authoritative Sprint 18 specification explicitly requires integration" (it does not). The three:

1. **Sprint 16's `agreement_dispute.status = 'restricted'`** — meaningful only within one specific dispute's own lifecycle, lifted only by `AgreementDisputeService.liftRestriction`.
2. **Sprint 18A's `RelationshipService.restrict`** — marks one specific cooperative relationship `restricted`, admin-gated via `isAdminRole` alone (no internal-role capability model existed yet at that point in the roadmap).
3. **Sprint 18's `AdminRestrictionService`** (new) — a general-purpose, admin-initiated restriction (`payment_activity`/`new_agreement_creation`/`payout`) targeting any resource (`user_account`, `business_profile`, `agreement`, …) independent of any dispute or relationship, gated by this sprint's own fine-grained internal-role capability model. Explicitly excludes account suspension (Sprint 6A's `AdminService.suspendUser`/`reactivateUser`, reused unchanged) and never touches the `relationship`/`agreement_dispute` tables at all.

No code in this sprint reads or writes `relationship`/`relationship_participant`/`agreement_dispute` — verified structurally (no import of `src/lib/relationships/*` or `src/lib/disputes/agreementDisputeService.ts`'s write methods appears anywhere in `src/lib/admin/{adminRestrictionService,retentionHoldService,appealService,supportCaseService,adminRoleService}.ts`), the same "guarantee is structural, not just tested" precedent Sprint 6A itself established for agreement/signature/PDF tables.

**The internal-admin-role/capability model** (`admin_role_assignment`, `src/lib/admin/adminCapabilities.ts`) mirrors Sprint 4's business-staff role/capability split exactly, applied to platform-level staff instead of per-business staff: `support`/`compliance`/`fraud_reviewer` each get a fixed `DEFAULT_INTERNAL_ROLE_CAPABILITIES` subset; `admin` always has every capability (the same structural-bypass precedent as Sprint 4's own "owner"). This sits *on top of* Sprint 6A's `PlatformRole` (member/platform_admin/platform_owner) — a `platform_admin` with no internal role assignment at all has zero Sprint 18 capabilities (fails closed), and `platform_owner` bypasses every capability check unconditionally, matching every other Owner-bypass precedent in this codebase (`changeUserRole`, `LedgerAdminService`, etc.). Role assignment/revocation is itself Owner-only, mirroring `changeUserRole`'s identical gating.

**A real authorization gap was caught by this sprint's own test suite, not left latent**: `AdminRestrictionService.lift`'s own restriction-type capability check (e.g. `restrict_payment_activity`) would have blocked an appeal reviewer — typically `compliance`, which deliberately does **not** hold restriction-placement capabilities — from ever completing an "overturned" appeal decision that lifts the restriction it's overturning. Fixed by adding `AdminRestrictionService.liftAsAppealOutcome`, called only by `AppealService.decideAppeal`, which deliberately skips the restriction-type check: `decideAppeal` has already independently verified the caller holds `manage_appeal` *and* is this specific appeal's assigned (structurally distinct-from-the-original-decision-maker) reviewer — sufficient authorization on its own, and requiring the reviewer to *also* separately hold the original restriction's own placement capability would defeat the entire purpose of independent appellate review.

**Retention/legal holds are fully correct and ready but have no real caller yet.** `RetentionHoldService.hasActiveHold` is exactly the query a future scheduled-deletion/minimization job must consult before purging anything — but **no such job exists anywhere in this codebase, in any prior sprint**. This is documented explicitly as a known limitation (see the completion report), not silently assumed complete; the natural owner is Sprint 20 ("closed beta readiness... retention verification" per this document's own dependency graph) or a dedicated future data-lifecycle sprint.

**Appeals reuse three prior-sprint mechanisms verbatim, inventing none of them**: Sprint 10's `LedgerAdminService.postAdjustment` (Owner-gated, never re-implemented or bypassed — a non-Owner reviewer's decision still records, but any attached ledger adjustment is rejected by that service's own existing gate), Sprint 17's `NotificationService.notify` (one new event type, `appeal_decided`, classified critical per master spec §30's explicit "notify the user by email" requirement), and the same free-text-`reason`/no-fault-field pattern Sprint 16 already established for "the platform must not adjudicate legal liability."

**"Review fraud alerts" has no backing data source** — no `fraud_alert` (or equivalent) table exists anywhere in this codebase; that is Sprint 19's own future scope. The capability is still declared (`review_fraud_alert`) so Sprint 19 has an existing capability to gate against rather than inventing a competing one. **"Review payment failures" and "review disputes"/"review verification status"/"review audit logs"** are all satisfied by thin, read-only, admin-authorized wrappers (`AdminCaseReviewService`) around existing prior-sprint data — Sprint 3's `VerificationService.getVerificationState`, Sprint 16's own dispute repositories (not the party-gated services, which would reject an admin who isn't a party), and a new read-only `audit_event` query reader mirroring Sprint 6A's own `AdminOverviewReader` precedent. Read-by-ID only in this pass, not a full cross-platform listing dashboard — a reasonable future enhancement, not built here.
