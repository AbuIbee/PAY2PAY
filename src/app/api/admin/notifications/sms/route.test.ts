...
function getWithCookie(sessionToken?: string) {
  ...
  return new NextRequest("http://localhost/api/admin/notifications/sms", { method: "GET", headers });
}

describe("GET /api/admin/notifications/sms — unauthorized direct API access is rejected", () => {
  ...
  function handler() {
    return withErrorHandling("admin_sms_delivery_list", createSmsDeliveryListHandler(authCtx.authService, adminCtx.smsDeliveryAdminService));
  }
