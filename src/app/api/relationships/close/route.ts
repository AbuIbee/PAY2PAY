...
const closeSchema = z.object({ relationshipId: z.string().uuid() });

export function createRelationshipCloseHandler(authService: AuthService, relationshipService: RelationshipService) {
  return async function handleClose(request: NextRequest): Promise<Response> {
    ...
    const relationship = await relationshipService.close(parsed.data.relationshipId, userId);
    return NextResponse.json({ relationship }, { status: 200 });
  };
}

async function handleClose(request: NextRequest): Promise<Response> {
  return createRelationshipCloseHandler(getAuthService(), getRelationshipService())(request);
}

export const POST = withErrorHandling("relationship_close", handleClose);
