...
export function createTotpConfirmHandler(authService: AuthService, mfaService: MfaService) {
  return async function handleConfirm(request: NextRequest): Promise<Response> {
    ...
    await mfaService.confirmTotpEnrollment(userId, parsed.data.code);
    return NextResponse.json({ status: "ok" }, { status: 200 });
  };
}

async function handleConfirm(request: NextRequest): Promise<Response> {
  return createTotpConfirmHandler(getAuthService(), getMfaService())(request);
}

export const POST = withErrorHandling("auth_mfa_totp_confirm", handleConfirm);
