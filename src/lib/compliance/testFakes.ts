...
import { BetaInviteService, type BetaInviteCodeRecord, type BetaInviteRepository } from "./betaInviteService";
...
export class InMemoryBetaInviteRepository implements BetaInviteRepository {
  private byCode = new Map<string, BetaInviteCodeRecord>();

  async insert(input: { code: string; createdByUserId: string; note: string | null }): Promise<BetaInviteCodeRecord> {
    const record: BetaInviteCodeRecord = { id: randomUUID(), createdAt: new Date(), usedByUserId: null, usedAt: null, ...input };
  ...
  async peekCode(code: string): Promise<BetaInviteCodeRecord | null> {
    ...
    return record && !record.usedByUserId ? record : null;
  }

  /** Synchronous, no-await-before-check — mirrors a real DB's atomic `UPDATE ... WHERE used_by_user_id IS NULL`. */
  async claimCode(code: string, usedByUserId: string): Promise<BetaInviteCodeRecord | null> {
    const record = this.byCode.get(code);
    if (!record || record.usedByUserId) return null;
...
export function createTestBetaInviteService() {
  const invites = new InMemoryBetaInviteRepository();
  const betaInviteService = new BetaInviteService({ invites });
  return { betaInviteService, invites };
}
