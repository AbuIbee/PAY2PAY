...
describe("computeEnvironmentStatus", () => {
  ...
  it("always reports payment/KYC providers as sandbox and email/SMS as console_log_only, regardless of APP_ENV — this codebase has no live adapter for any of them yet", () => {
    for (const appEnv of ["development", "test", "staging", "production"] as const) {
      ...
      expect(status.emailDelivery).toBe("console_log_only");
